...
# **¿QUIÉNES CONFÍAN EN LOS MEDIOS EN LAS AMÉRICAS**
...
En este taller vamos a replicar el informe 142 de la Serie Perspectivas desde el Barómetro de las Américas. Se puede ver aquí la lista de los [Insights](https://www.vanderbilt.edu/lapop-espanol/perspectivas/). El informe 142 se puede conseguir en español [aquí](https://www.vanderbilt.edu/lapop/insights/IO942es.pdf).
Se asume que los participantes tienen un conocimiento básico de R y RStudio. A lo largo de este taller, explicaremos, paso a paso, los procedimientos en RStudio para producir las tablas y gráficos que se ven en los informes. En estos informes, estas tablas y gráficos han sido producidos en STATA. 

En resumen, este informe presenta el promedio de confianza en los medios de comunicación en las Américas y dos modelos explicativos de la confianza en los medios usando factores socioeconómicos y factores políticos.
...
Pueden descargar los datos de manera libre en [Acceso](http://datasets.americasbarometer.org/database/login.php)
...
Debido a que estas bases de datos son muy grandes (42MB en formato zip), para este taller vamos a trabajar con una versión reducida, que incluye los países y años seleccionados en el informe. Esta base de datos se puede descargar [aquí](INCLUIR LINK)


# INSTALANDO LOS PAQUETES NECESARIAS EN R
...
```{r}
graf1_pond <- ggplot(tab.b37r, aes(x=reorder(pais, media), y=media)) +
  geom_bar(width=0.5, fill="purple", colour="black", stat="identity")+
...
```{r}
modelo1.pond <- svyglm(b37 ~ mujer + edrr + size +
...
```{r}
modelo2.pond <- svyglm(b37 ~ it1r + q5a01 + newsattn + useinternet + 
...
```{r}
export_summs(modelo1.pond, modelo2.pond)
```
Para reproducir el gráfico 2, podemos usar el siguiente código:
```{r}
plot_summs(modelo2.pond, coefs=c("Confianza interpersonal" = "it1r", 
